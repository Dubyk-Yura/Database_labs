...
def create_app(app_config: Dict[str, Any]) -> Flask:
    ...
    """
    _process_input_config(app_config)
    app = Flask(__name__)
...
def _init_swagger(app: Flask) -> None:
    # A-lia Swagger
    restx_api = Api(app, title='Yura test backend',
...
def _process_input_config(app_config: Dict[str, Any]) -> None:
    """
    Processes input configuration
    :param app_config: Flask configuration
    """
    # Get root username and password
    root_user = os.getenv(MYSQL_ROOT_USER, os.getenv("DB_USER"))
    root_password = os.getenv(MYSQL_ROOT_PASSWORD, os.getenv("DB_PASSWORD"))
